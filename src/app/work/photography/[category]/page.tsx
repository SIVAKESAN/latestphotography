...
export async function generateMetadata({
  ...
  const categories = await DataService.getCategories('photography');
  const cat = categories.find(c => c.slug === category);

  const title = cat ? `${cat.name} Photography` : 'Photography Stories';
  return {
    title,
    description: cat?.description || "Photography visual archive from LATEST PHOTOGRAPHY.",
  };
}
  ...
  return (
    <div className="min-h-screen bg-white pt-28 pb-32">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <Link
          href="/work"
          className="inline-flex items-center gap-2 text-xs uppercase tracking-wider text-slate-600 hover:text-blue-600 font-medium transition-colors mb-10"
        >
        ...
        <div className="max-w-3xl mb-14">
          <span className="text-xs font-mono uppercase tracking-[0.25em] text-blue-600 font-semibold">
            PHOTOGRAPHY // {currentCat.name.toUpperCase()}
          </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project) => (
            <article key={project.id} className="group flex flex-col bg-white border border-slate-200 rounded-lg p-3 shadow-xs hover:shadow-md transition-shadow duration-300">
              <Link
                href={`/work/photography/story/${project.slug}`}
                className="block relative aspect-[4/5] overflow-hidden rounded-md bg-slate-100"
              >
                <Image
              ...
              <div className="pt-3.5 px-1 pb-1 flex items-start justify-between">
                <div>
                  <h2 className="text-lg md:text-xl font-bold text-slate-900 group-hover:text-blue-600 transition-colors">
                    <Link href={`/work/photography/story/${project.slug}`}>
                      {project.title}
                    </Link>
                  </h2>
